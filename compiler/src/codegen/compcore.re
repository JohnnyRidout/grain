...
let tracepoint = (wasm_mod, env, n) =>
  /*Concatlist.t_of_list [
      Ast.Const(const_int32 wasm_mod n);
...
let cleanup_locals = (wasm_mod, env: codegen_env, arg): Expression.t => {
  ...
  /* (set_swap env 0) @ (get_swap env 0) +@ [
       ...
       Ast.Drop;
     ] @ (cleanup_local_slot_instructions wasm_mod env) @ (get_swap env 0) +@ [
...
let error_if_true = (wasm_mod, env, cond, err, args) =>
  /* Ast.If([],
     Concatlist.mapped_list_of_t add_dummy_loc (call_error_handler wasm_mod env err args),
...
/* tup @ (untag TupleTagType) @ set_swap @ get_swap @ (compile_imm wasm_mod env imm) +@ [
...
/* compiled_num_elts +@ [
     ...
     error_if_true wasm_mod env InvalidArgument [num_elts];
...
let allocate_array_init = (wasm_mod, env, num_elts, init_f) => {
  let get_arr_addr = () => get_swap(wasm_mod, env, 0);
  let set_arr_addr = set_swap(wasm_mod, env, 0);
  let get_loop_counter = () => get_swap(wasm_mod, env, 1);
  let set_loop_counter = set_swap(wasm_mod, env, 1);
...
let compile_prim2 = (wasm_mod, env: codegen_env, p2, arg1, arg2): Expression.t => {
  ...
  let swap_set = set_swap(wasm_mod, env, 0);
  let swap_tee = tee_swap(wasm_mod, env, 0);
  let overflow_safe = arg =>
    /* let compiled_swap_get = get_swap ~ty:Type.int64 wasm_mod env 0 in
       let compiled_swap_set = set_swap ~ty:Type.int64 wasm_mod env 0 in
       let compiled_swap_tee = tee_swap ~ty:Type.int64 wasm_mod env 0 in */
    Expression.unary(
      wasm_mod,
      Op.wrap_int64,
      check_overflow(wasm_mod, env, arg),
    );
  /* Expression.block wasm_mod (gensym_label "overflow_safe") [
       check_overflow wasm_mod env arg;
     ] in */
  /* instrs @
     compiled_swap_set @
...
let compile_imports = (wasm_mod, env, {imports} as prog) => {
...
/* ignore @@ Export.add_memory_export wasm_mod "mem" "memory" */
/* (List.append
...
let compile_elems = (wasm_mod, env, prog) =>
  /* let table_size = compute_table_size env prog in
     let offsets = [
       Expression.global_get wasm_mod "global_0" Type.int32
     ] in
     Memory.set_memory wasm_mod 0 256 "memory" ["main"] [false] offsets [256] false */
  ();
/* let table_size = compute_table_size env prog in
   let open Wasm.Ast in
   [
...
/* Function.set_start wasm_mod ret */
...
let validate_module = (~name=?, wasm_mod: Module.t) =>
  ...
  | Assert_failure(_) =>
    /* let reparsed = reparse_module wasm_mod in
       (try
